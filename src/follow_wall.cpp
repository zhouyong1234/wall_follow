...
void laser_callback(const sensor_msgs::LaserScan::ConstPtr &scan_msg) {
  ...
  for(size_t i = 0, j = 1368; i < range_size * 1 / 20, j < range_size; i++, j++)
  {
      if (laser_rays[i] < z[2])
      {
          z[2] = laser_rays[i];
  ...
  for(size_t i = 72; i < range_size * 1 / 4; i++)
  {
      if(i < range_size * 3 / 20)
      {
          if(laser_rays[i] < z[3])
          {
              z[3] = laser_rays[i];
      ...
      else if(i >= range_size * 3 / 20 && i < range_size * 1 / 4)
      {
          if(laser_rays[i] < z[4])
          {
              z[4] = laser_rays[i];
  ...
  for(size_t i = 1080; i < range_size * 19 / 20; i++)
  {
      if(i < range_size * 17 / 20)
      {
          if(laser_rays[i] < z[0])
          {
              z[0] = laser_rays[i];
      ...
      else if(i >= range_size * 17 / 20 && i < range_size * 19 / 20)
      {
          if(laser_rays[i] < z[1])
          {
              z[1] = laser_rays[i];
...
int main(int argc, char **argv) {
  ...
  laser_subscriber = n.subscribe("/combined_scan", 1000, laser_callback);
//   ros::spin();
