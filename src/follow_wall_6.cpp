...
void laser_callback(const sensor_msgs::LaserScan::ConstPtr &scan_msg) {
  ...
  for (size_t i = 0; i < range_size * 3 / 12; i++) {

    // rays < 120, laser rays to the front
    if (i < range_size / 12) {
    ...
    // rays >= 120 and rays < 240, laser rays to the front-left side
    else if (i >= range_size / 12 && i < range_size * 2 / 12) {
      // get the smallest (closest) laser range value
      if (laser_rays[i] < z[1]) {
        z[1] = laser_rays[i];
      }
    }
    // rays >= 240 and rays < 360, laser rays to the left
    else if (i >= range_size * 2 / 12 && i < range_size * 3 / 12) {
      // get the smallest (closest) laser range value
      if (laser_rays[i] < z[2]) {
        z[2] = laser_rays[i];
      }
  ...
  for (size_t i = range_size * 9 / 12; i < range_size; i++)
  {
    // rays >= 1080 and rays < 1200, laser rays to the right
    if (i < range_size * 10 / 12) {
      // get the smallest (closest) laser range value
      if (laser_rays[i] < z[3]) {
        z[3] = laser_rays[i];
      }
    }
    // rays > 1200 and rays <= 1320, laser rays to the front-right side
    else if (i >= range_size * 10 / 12 && i < range_size * 11 / 12) {
      // get the smallest (closest) laser range value
      if (laser_rays[i] < z[4]) {
        z[4] = laser_rays[i];
      }
    }
    // rays > 1320 and rays <= 1440, laser rays to the front
    else if(i >=  range_size * 11 / 12 && i < range_size){
      // get the smallest (closest) laser range value
      if (laser_rays[i] < z[5]){
        z[5] = laser_rays[i];
      }
...
// logic block 1:
  if (z[0] > d && z[1] > d && z[2] > d && z[3] > d && z[4] > d && z[5] > d) {
    ROS_INFO("case 1: no obstacles detected");
    state = 0; // find wall: turn right while moving ahead
  } else if (z[0] < d && z[1] > d && z[2] > d && z[3] > d && z[4] > d && z[5] < d) {
  ...
  // logic block 3:
    ...
    state = 5; // find wall: turn right while moving ahead
  } else if (z[0] < d && z[1] > d && z[2] < d && z[3] > d && z[4] < d && z[5] < d) {
